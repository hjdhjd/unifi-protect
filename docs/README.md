...
| [LivestreamOptions](interfaces/LivestreamOptions.md) | Options for [Camera.livestream](interfaces/Camera.md#livestream) - the stream-affecting `source` selector (a quality channel or a secondary lens) and tuning (segment length, chunk size, opt-in timestamps) without `cameraId` (the camera supplies its own id), plus an optional abort signal that disposes the returned subscription and an optional `urgency` closure feeding the pool's resilient recovery. |
...
| [LivestreamSubscriptionDisposedPayload](interfaces/LivestreamSubscriptionDisposedPayload.md) | Payload published on [channels.livestreamSubscriptionDisposed](variables/channels.md#property-livestreamsubscriptiondisposed). `stats` carries the subscription's final delivery counters. |
