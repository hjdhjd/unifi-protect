...
[Home](../README.md) / Light

# Interface: Light

A light projection. Inherits the read-through getters, live `observe`, and write-through `update` from [DeviceProjection](DeviceProjection.md); light-specific behavior (level,
motion activation) is expressed through `update` payloads rather than bespoke methods.

## Extends

- [`DeviceProjection`](DeviceProjection.md)\<[`ProtectLightConfig`](../type-aliases/ProtectLightConfig.md)\>
...
```ts
observe(opts?): AsyncGenerator<Light>;
```
...
```ts
peek(): 
  | Readonly<ProtectLightConfigInterface>
  | undefined;
```
...
#### Returns
  ...
  \| `Readonly`\<[`ProtectLightConfigInterface`](ProtectLightConfigInterface.md)\>
  \| `undefined`
...
```ts
update(payload, opts?): Promise<Light>;
```
