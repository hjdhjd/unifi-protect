...
[Home](../README.md) / Sensor

# Interface: Sensor

A sensor projection. Inherits the read-through getters, live `observe`, and write-through `update` from [DeviceProjection](DeviceProjection.md); sensor-specific behavior
(sensitivity, mount type, thresholds) is expressed through `update` payloads.

## Extends

- [`DeviceProjection`](DeviceProjection.md)\<[`ProtectSensorConfig`](../type-aliases/ProtectSensorConfig.md)\>
...
| <a id="modelkey"></a> `modelKey` | `readonly` | `"sensor"` | `"sensor"` | The device category discriminant, fixed by the concrete subclass. | [`DeviceProjection`](DeviceProjection.md).[`modelKey`](DeviceProjection.md#modelkey) | - |
...
```ts
observe(opts?): AsyncGenerator<Sensor>;
```
...
```ts
peek(): 
  | Readonly<ProtectSensorConfigInterface>
  | undefined;
```
...
#### Returns
  ...
  \| `Readonly`\<[`ProtectSensorConfigInterface`](ProtectSensorConfigInterface.md)\>
  \| `undefined`
...
```ts
update(payload, opts?): Promise<Sensor>;
```
