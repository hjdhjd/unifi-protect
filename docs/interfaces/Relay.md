...
- [`DeviceProjection`](DeviceProjection.md)\<[`ProtectRelayConfig`](../type-aliases/ProtectRelayConfig.md)\>
...
```ts
observe(opts?): AsyncGenerator<Relay>;
```
...
```ts
peek(): 
  | Readonly<ProtectRelayConfigInterface>
  | undefined;
```
...
#### Returns
  ...
  \| `Readonly`\<[`ProtectRelayConfigInterface`](ProtectRelayConfigInterface.md)\>
  \| `undefined`
...
```ts
toggleOutput(outputId, opts?): Promise<void>;
```
...
pre-validated here - an unknown id surfaces as the controller's classified [FatalError](../classes/FatalError.md), consistent with the write-through, controller-as-authority grain.

#### Parameters

| Parameter | Type | Description |
| ------ | ------ | ------ |
| `outputId` | `number` | The numeric id of the output to toggle (as reported on each `outputs[]` entry). |
| `opts` | \{ `signal?`: `AbortSignal`; \} | Optional abort signal. |
| `opts.signal?` | `AbortSignal` | - |
...
The classified `FatalError` on a non-2xx (e.g., an unknown output id), or a transport-level `ProtectError` on failure.
...
```ts
update(payload, opts?): Promise<Relay>;
```
