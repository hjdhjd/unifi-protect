...
A fob projection. Inherits the read-through getters, live `observe`, and write-through `update` / `reboot` from [DeviceProjection](DeviceProjection.md). A fob is an input device: its
...
- [`DeviceProjection`](DeviceProjection.md)\<[`ProtectFobConfig`](../type-aliases/ProtectFobConfig.md)\>
...
```ts
observe(opts?): AsyncGenerator<Fob>;
```
...
```ts
peek(): 
  | Readonly<ProtectFobConfigInterface>
  | undefined;
```
...
#### Returns
  ...
  \| `Readonly`\<[`ProtectFobConfigInterface`](ProtectFobConfigInterface.md)\>
  \| `undefined`
...
```ts
update(payload, opts?): Promise<Fob>;
```
